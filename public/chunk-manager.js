...
class ChunkManager {
    constructor(file) {
        this.file = file;
        this.MAIN_CHUNK_SIZE = 50 * 1024 * 1024; // 50MB
        this.SUB_CHUNK_SIZE = 1 * 1024 * 1024;    // 1MB

        this.mainChunks = [];
        this.completedSubChunks = new Set();
        this.failedSubChunks = new Set();
     ...
     */
    getNextMainChunk() {
        return this.mainChunks.find(chunk => chunk.status === 'pending');
    }

    /**
     * メインチャンクのサブチャンクを取得
     */
    getSubChunks(mainChunkId) {
     ...
     * 次の送信するサブチャンクを取得
     */
    getNextSubChunk(mainChunkId) {
        const subChunks = this.getSubChunks(mainChunkId);
        return subChunks.find(chunk => chunk.status === 'pending' && !this.failedSubChunks.has(chunk.id));
    }

    /**
     * サブチャンク完了を記録
     */
    markSubChunkCompleted(subChunkId, checksum) {
